  ...
  const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Опиши 2-3 актуальных тренда/ситуации (последние ${freshnessDays} дней, ${currentYear} год) по теме "${topic}" применительно к региону ${region}. Фокус: домофоны, IP-видеонаблюдение, шлагбаумы для ЖК, дворов и частных домов. ИЗБЕГАЙ тем: государственный проект "Безопасный город", биометрические системы с анализом походки/вен/отпечатков, Единая биометрическая система (ЕБС), солнечные панели, электромонтаж, пожарная сигнализация. Включи: типичные проблемы жильцов и УК Кубани, технологические новинки ${currentYear} года в нашей нише. Не выдумывай конкретные адреса, даты, цифры и номера ФЗ — пиши обобщённо. НЕ упоминай 2024 год.`,
        },
      ],
    }),
  });
...
function cleanMarkdown(s: string): string {
  ...
  // 1. Удалить служебные tool-arg метки и всё после них
  ...
  // 2. Заголовки в **...** → чистый заголовок
  out = out.replace(/^(#{1,6})\s*\*+\s*(.+?)\s*\*+\s*$/gm, "$1 $2");

  // 3. Пустые заголовки и пустые буллеты
  out = out.replace(/^#{1,6}\s*$/gm, "");
  out = out.replace(/^[-*]\s*$/gm, "");

  // 4. Абзац, начинающийся с ** но не закрытый на той же строке — снимаем открывающую **
  //    (AI часто пишет "**Текст\nещё текст" без закрытия)
  out = out.replace(/^\*\*(?!\*)([^\n*]*?)$/gm, (m, p1) => p1);

  // 5. Строка, заканчивающаяся на ** без открывающей пары на той же строке — снимаем хвост
  out = out.replace(/^([^\n*]*?)\*\*\s*$/gm, (m, p1) => {
    // если в строке нет открывающей **, убираем хвостовые
  ...
  // 7. 3+ звёздочек подряд → 2
  out = out.replace(/\*{3,}/g, "**");

  // 8. Балансировка ** по всему тексту: если число вхождений нечётное — удаляем последнее
  const stars = (out.match(/\*\*/g) || []).length;
  if (stars % 2 === 1) {
    out = out.replace(/\*\*([^*]*)$/, "$1");
  }

  // 9. Решётки в начале строк, если не часть заголовка (### без пробела или "###слово")
  out = out.replace(/^(#{1,6})([^\s#])/gm, "$1 $2");

  // 10. Удалить одиночные #, ##, ### в конце текста
  out = out.replace(/\n+#{1,6}\s*$/g, "");
