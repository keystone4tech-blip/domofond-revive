...
export const generateProposalDocx = async (data: ProposalData): Promise<Blob> => {
  ...
  if (elevPrice > 0) {
    tariffTextArray.push(`Техническое обслуживание лифтовых камер видеонаблюдения – ${elevPrice} руб.`);
  }

  if (gatePrice > 0) {
    tariffTextArray.push(`За обслуживание вызывных блоков на калитках – ${gatePrice} руб.`);
  }
    ...
    sections: [{
      ...
      children: [
        ...
        ...(() => {
          ...
          // 1. Домофон (Показываем только если есть умные домофоны)
          if (calculation.smartIntercoms > 0) {
          ...
          if (calculation.gates > 0) {
            items.push(new Paragraph({
              children: [new TextRun({ text: `${sectionIdx++}. Установка или модернизация вызывных блоков на калитках за счет компании (бесплатно)`, bold: true })]
            }));
            items.push(new Paragraph({
              children: [new TextRun({ text: `${sectionIdx++}. Техническое обслуживание вызывного блока на калитке`, bold: true })]
            }));
          }
        ...
        ...[
          ...(calculation.smartIntercoms > 0 ? [`Модернизация установленного домофонного оборудования с заменой на «умный» IP-домофон за счет компании ООО «Домофондар» (центральный блок вызова, блоки питания, коммутаторы, считыватели, замки, доводчики) — бесплатно.`] : []),
          ...(calculation.additionalCameras > 0 ? [`Установка системы видеонаблюдения на придомовой территории за счет компании ООО «Домофондар» (количество дополнительных камер: ${calculation.additionalCameras} шт.), работа камер в одном приложении с карой домофона «Умный дом» архив 5 суток.`] : []),
          ...
          ...(calculation.gates > 0 ? [`Модернизация установленного домофонного оборудования на калитках с заменой на «умный» IP-домофон за счет компании ООО «Домофондар» (центральный блок вызова, блоки питания, коммутаторы, считыватели, замки, доводчики) — бесплатно.`] : []),
          tariffText,
