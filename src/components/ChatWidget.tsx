...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    console.log("[Чат-виджет] Запуск 8-секундного таймера авто-открытия...");

    // Раскрываем чат через 8 секунд после монтирования
    const openTimer = setTimeout(() => {
      console.log("[Чат-виджет] Автоматическое открытие виджета для привлечения внимания");
      setOpen(true);
      setAutoOpened(true);

      // Запускаем таймер автоматического сворачивания через 10 секунд привлечения внимания
